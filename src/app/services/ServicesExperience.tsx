...
const SERVICE_DATA: Service[] = [
  {
    ...
    subtitle: "Applications web & iOS",
    headline: "Optimiser la performance en ski nordique grâce aux solutions numériques",
    description:
      "Applications, outils et workflows sur mesure pour athlètes, coachs et marques. Chaque module est pensé pour accélérer l'analyse et aider à prendre de meilleures décisions.",
    deliverables: [
      "Application mobile terrain (tests, notes, décisions)",
      "Espace web pour centraliser les chiffres clés",
      "Parcours lisible pour convertir plus de prises de contact",
    ],
    process: [
      "Audit rapide de vos usages terrain et objectifs",
      "Prototype clair des écrans et indicateurs clés",
      "Déploiement + itérations continues selon les retours",
    ],
    equipmentTitle: "Briques digitales",
    equipment: ["Application SkiTrack", "Dashboard staff & marque", "Automatisation des rapports"],
  ...
  },
  {
    id: "photo",
    title: "Service photo",
    subtitle: "Production visuelle premium",
    headline: "Des images fortes pour valoriser vos produits, vos athlètes et votre marque",
    description:
      "Le service photo n'est pas limité au ski nordique: il couvre aussi les besoins visuels de campagnes, produits et événements. Objectif: un rendu premium, cohérent et exploitable immédiatement.",
    deliverables: [
      "Shooting terrain, événement et produit",
      "Tri éditorial + traitement colorimétrique",
      "Livraison en galerie privée prête à diffuser",
    ],
    process: [
      "Brief d'intention et objectifs d'image",
      "Production terrain avec sélection continue",
      "Livraison galerie + exports web, social et print",
    ],
    equipmentTitle: "Matériel photo disponible",
    equipment: ["Boîtiers plein format", "Optiques G Master / G", "Workflow de livraison rapide"],
...
const WEB_APP_SHOWCASE = [
  ...
  },
  {
    ...
    description: "Interfaces utiles pour piloter les données et guider l'action.",
...
export default function ServicesExperience({ photoImages }: ServicesExperienceProps) {
  ...
  const visuals = useMemo(() => {
    ...
    return {
      webios: ["/services/solutionwebios.jpg", "/services/biathlon-pour.jpg", "/hero-home.jpg", "/services/solutionwebios.jpg"],
      photo: [
        "/services/photo/24-DSC03738.JPG",
        pick(0),
        pick(1),
  ...
  return (
    <div className="space-y-9 sm:space-y-11">
      <div className="mx-auto max-w-3xl text-center" data-reveal style={reveal(40)}>
        ...
        <h2 className="mt-3 text-3xl font-semibold tracking-[-0.03em] text-ink sm:text-4xl lg:text-5xl">
          Des services alignés sur la performance et la conversion
        </h2>
        <p className="mt-4 text-ink/65">
          Un pôle numérique dédié au ski nordique et un pôle photo premium pour renforcer
          votre image, sur et hors du terrain.
        </p>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        {SERVICE_DATA.map((service, index) => {
          ...
          const image = service.id === "webios" ? "/services/solutionwebios.jpg" : "/services/photo/24-DSC03738.JPG";
      ...
      <section className="relative overflow-hidden rounded-[34px] border border-white/8 bg-[#081019] p-5 sm:p-7 lg:p-8" data-reveal style={reveal(60)}>
        ...
        <div key={active} className="animate-reveal relative z-10 grid gap-6 xl:grid-cols-[1fr_1fr] xl:items-start">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {PHOTO_SHOWCASE_METRICS.map((metric) => (
                    ...
                    >
                      ...
                      <p className="text-[10px] uppercase tracking-[0.16em] text-white/65">{metric.label}</p>
                    </div>
                  ))}
                </div>
              </div>
                  ...
                  {WEB_APP_SHOWCASE.map((app) => (
                    ...
                    >
                      ...
                      <div className="absolute inset-x-0 bottom-0 p-4">
                        <p className="text-[10px] uppercase tracking-[0.2em] text-white/70">{app.subtitle}</p>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <article className="rounded-3xl border border-line bg-white p-5 shadow-[0_10px_28px_rgba(35,48,54,0.09)] sm:p-6" data-reveal style={reveal(80)}>
        ...
        <article className="rounded-3xl border border-line bg-white p-5 shadow-[0_10px_28px_rgba(35,48,54,0.09)] sm:p-6" data-reveal style={reveal(140)}>
        ...
        <article className="rounded-3xl border border-line bg-white p-5 shadow-[0_10px_28px_rgba(35,48,54,0.09)] sm:p-6" data-reveal style={reveal(200)}>
      ...
      {active === "photo" && (
        <section data-reveal style={reveal(70)}>
          <div className="mb-5 flex items-end justify-between">
            <div>
              <p className="text-[10px] uppercase tracking-[0.28em] text-ink/55 sm:text-[11px]">Matériel</p>
          ...
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-5">
            {PHOTO_EQUIPMENT.map((item, index) => (
              <a
                key={item.id}
                href={item.href}
                target="_blank"
              ...
              >
                ...
                <div className="p-3.5 sm:p-4">
                  <p className="text-[9px] uppercase tracking-[0.22em] text-ink/45 sm:text-[10px]">{item.category}</p>
                  <p className="mt-0.5 text-sm font-semibold text-ink sm:text-[15px]">{item.name}</p>
                  <p className="mt-1 text-[10px] text-deep/60 transition group-hover:text-deep">Voir sur Sony →</p>
                </div>
              </a>
            ))}
          </div>
